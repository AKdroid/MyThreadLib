...
ucontext_t exitguard;
char exitstack[16384];
char clean_stack[16384];
...
void cleanup(void){
    ...
    if(DEBUG)
        printf("MYTHREAD: MyThreadCleanup # Destroying all remaining threads\n");

    do{
        temp = (uthread*)dequeue(&(handler.ready_h),&(handler.ready_t));
    }
    while(temp != NULL);
...
void MyThreadInitExtra(){
    ...
    uthread* t;
    if(DEBUG)
        printf("MYTHREAD: MyThreadInitExtra # Initializing Manager\n");

    init_manager(&mgr);
    init_smanager(&smgr);
    t = init_thread(&mgr,&(exitguard),RUNNING,dummy,0);
    
    if(DEBUG)
        printf("MYTHREAD: MyThreadInitExtra # Root Thread initialized with ID: %d\n",t->id);

    //Initialize the ready queue
    init_queue(&(handler.ready_h),&(handler.ready_t));

    if(DEBUG)
        printf("MYTHREAD: MyThreadInitExtra # Ready Queue Initialized\n");
    ...
    if(DEBUG){
        print_ready_queue();
        printf("MYTHREAD: MyThreadInitExtra # Root thread added to the ready queue.\n");
    }


    getcontext(&exitguard);
    
    exitguard.uc_link = &(handler.main_ctx);
    exitguard.uc_stack.ss_sp = exitstack;
    exitguard.uc_stack.ss_size = 16384;
    
    makecontext(&exitguard,MyThreadExit,0);

    getcontext(&(handler.main_ctx));
    handler.main_ctx.uc_link = NULL;
    handler.main_ctx.uc_stack.ss_sp = clean_stack;
    handler.main_ctx.uc_stack.ss_size = 16384;

    makecontext(&(handler.main_ctx),cleanup,0);
...
void MyThreadInit(void(*start_funct)(void *), void *args){
    ...
    t = init_thread(&mgr,&(exitguard),RUNNING,start_funct,args);
    ...
    // Initialize exitguard
    getcontext(&exitguard);

    exitguard.uc_link = NULL;
    exitguard.uc_stack.ss_sp = exitstack;
    exitguard.uc_stack.ss_size = 16384;
...
MyThread MyThreadCreate(void(*start_funct)(void *), void *args){
    ...
    new_thread = init_thread(&mgr, &exitguard, READY, start_funct, args);
